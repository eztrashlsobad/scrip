...
function TDS:Addons()
    local url = "https://google.com" --"https://api.jnkie.com/api/v1/luascripts/public/57fe397f76043ce06afad24f07528c9f93e97730930242f57134d0b60a2d250b/download"
...
function TDS:Loadout(...)
    ...
    local StateReplicators = ReplicatedStorage:FindFirstChild("StateReplicators")
    local Assets = ReplicatedStorage:WaitForChild("Assets"):WaitForChild("Troops")
    ...
    for _, TowerName in ipairs(towers) do
        if TowerName and TowerName ~= "" then
            -- Stream the tower assets from the server
            pcall(function()
            ...
            local timeout = 5
            local elapsed = 0
            while not Assets:FindFirstChild(TowerName) and elapsed < timeout do
                task.wait(0.1)
                elapsed = elapsed + 0.1
            end

            if not Assets:FindFirstChild(TowerName) then
                Logger:Log("Timed out loading assets for: " .. TowerName)
            end
